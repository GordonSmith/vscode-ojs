...
import { html2notebook, type Notebook, compileKit, notebook2js } from "./compiler";
        ...
        try {
            ...
            if (!isObservableNotebook(contentStr)) {
                const proceed = await vscode.window.showWarningMessage(
                    "This file does not look like an Observable Notebook HTML. Try exporting anyway?",
                    { modal: true },
                    "Export",
                    "Cancel"
                );
            ...
            const defaultJsUri = targetUri.with({ path: targetUri.path.replace(/\.[Hh][Tt][Mm][Ll]$/i, ".js") });
            ...
            const jsModule = "// Generated by vscode-ojs: Observable Notebook Kit export\n"
                + "// Source: " + Commands.basename(targetUri.path) + "\n"
                + "const notebook = " + JSON.stringify(notebook, null, 2) + ";\n"
                + "export default notebook;\n";

            await vscode.workspace.fs.writeFile(saveUri, new TextEncoder().encode(jsModule));

            // Also offer to export compiled definitions as a JS module
            try {
                const compiledDefaultUri = saveUri.with({ path: saveUri.path.replace(/\.js$/i, ".definition.js") });
                const compiledSaveUri = await vscode.window.showSaveDialog({
                    defaultUri: compiledDefaultUri,
                    saveLabel: "Export compiled definitions (.definition.js)"
                });
                if (compiledSaveUri) {
                    const compiledJs = notebook2js(notebook);
                    await vscode.workspace.fs.writeFile(compiledSaveUri, new TextEncoder().encode(compiledJs));
                }
            } catch (err) {
                // If compilation fails, surface an info message but still succeed in main export
                vscode.window.showWarningMessage("Compiled export failed: " + err);
            }
